    ...
    <div className="w-full max-w-full overflow-hidden">
      <div className="min-h-[calc(100vh-8rem)] space-y-6 lg:space-y-8 w-full max-w-full px-4 sm:px-6">
        ...
        {Object.values(errors).some(Boolean) && (
          <div className="flex items-center gap-2 p-4 bg-accent border border-accent rounded-lg">
                    ...
                    >
                      <div className="flex items-center justify-between mb-3 min-w-0">
                        <div className="flex items-center gap-3 min-w-0 flex-1">
                          {chapter.status === 'completed' && <CheckCircle className="w-5 h-5 text-secondary-foreground flex-shrink-0" />}
                          {chapter.status === 'in_progress' && <Play className="w-5 h-5 text-primary flex-shrink-0" />}
                        ...
                        <div className="flex items-center gap-2 flex-shrink-0">
                          <Badge
                            variant="outline"
                            className={
                              chapter.difficulty === 'Beginner' ? 'border-secondary text-secondary-foreground' :
                              chapter.difficulty === 'Intermediate' ? 'border-primary text-primary' :
                              chapter.difficulty === 'Advanced' ? 'border-secondary text-secondary-foreground' :
                              chapter.difficulty === 'Expert' ? 'border-muted text-muted-foreground' :
                          ...
                          {chapter.status !== 'locked' && (
                            <span className="text-sm font-medium text-accent-foreground flex-shrink-0">
                      ...
                      <div className="flex items-start justify-between mb-3 min-w-0">
                        ...
                        <div className="text-right flex-shrink-0">
                          ...
                          <Badge
                            ...
                            className={
                              room.status === 'active' ? 'bg-secondary text-secondary-foreground' :
                              room.status === 'waiting' ? 'bg-muted text-muted-foreground' :
                  ...
                  currentLeaderboard.map((player) => (
                    <div key={player.rank} className={cn(
                      ...
                      <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${
                        ...
                        player.rank === 3 ? 'bg-muted text-white' :
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="flex items-center gap-2 text-xs text-muted-foreground min-w-0">
                          ...
                          <span className={`flex-shrink-0 ${player.change.startsWith('+') ? 'text-secondary-foreground' : 'text-muted-foreground'}`}>
                            {player.change}
                          </span>
                        </div>
                      </div>
                  ...
                  <>
                    ...
                    <div className="w-full max-w-full">
                      ...
                      <div className="grid grid-cols-7 gap-1 w-full max-w-full">
                        {currentPerformance.weeklyTrades.winRateTrend.map((rate, index) => (
                          <div
                            key={index}
                            className={`aspect-square rounded ${rate >= 75 ? 'bg-secondary' :
                                rate >= 60 ? 'bg-secondary' :
                                  rate >= 45 ? 'bg-secondary' :
                                    rate >= 30 ? 'bg-muted' : 'bg-muted'
                              }`}
                            title={`${rate}% win rate`}
                          />
                        ))}
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        ...
                        <span className={currentPerformance.weeklyTrades.totalPL >= 0 ? "text-secondary-foreground" : "text-muted-foreground"}>
