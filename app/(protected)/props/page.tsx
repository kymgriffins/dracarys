...
// Discount codes mapping from sidebar promotional offers
const discountCodes: Record<string, { percentage: number; code: string; description: string }> = {
  "FTMO": { percentage: 30, code: "DRACARYS30", description: "Challenge accounts" },
  "MyForexFunds": { percentage: 25, code: "DRACARYS25", description: "Evaluation phase" },
  "The5ers": { percentage: 20, code: "DRACARYS20", description: "Instant funding" },
  "TrueForexFunds": { percentage: 35, code: "DRACARYS35", description: "All accounts" },
  "FundingPips": { percentage: 15, code: "DRACARYS15", description: "Express accounts" }
};
        ...
        {propFirms.length === 0 ? (
          <Card className="border-dashed border-2 border-gray-300 bg-gray-50">
            <CardContent className="flex flex-col items-center justify-center py-16">
              ...
              <h3 className="text-xl font-semibold text-gray-900 mb-2">No prop firms yet</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {propFirms.map((firm) => (
                <Card key={firm.id} className="border-gray-200 hover:border-gray-300 transition-all duration-300 hover:shadow-xl hover:shadow-gray-200/50 bg-white">
                <CardHeader className="pb-3">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-gradient-to-br from-gray-900 to-gray-700 rounded-lg flex items-center justify-center overflow-hidden flex-shrink-0 shadow-sm">
                      {propFirmLogos[firm.provider_name as keyof typeof propFirmLogos] ? (
                        <img
                          ...
                          onError={(e) => {
                            ...
                            e.currentTarget.parentElement!.innerHTML = '<svg class="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"></path></svg>';
                          }}
                        />
                      ) : (
                    ...
                    <div className="min-w-0 flex-1">
                      <CardTitle className="text-base flex items-center gap-2 text-gray-900">
                        ...
                        <Badge variant={firm.is_active ? "default" : "secondary"} className="text-xs flex-shrink-0 bg-gray-100 text-gray-700 hover:bg-gray-200">
                          {firm.is_active ? "Active" : "Inactive"}
                        </Badge>
                      </CardTitle>
                      <CardDescription className="font-medium text-gray-600 text-sm truncate">
                        {firm.provider_name}
                      </CardDescription>
                    </div>
                  </div>
                ...
                <CardContent className="pt-0">
                  <div className="space-y-3">
                    {/* Discount Code Section - Most Prominent */}
                    {discountCodes[firm.provider_name] ? (
                      <div className="bg-gradient-to-br from-amber-50 via-yellow-50 to-amber-50 border border-amber-200 rounded-lg p-4 shadow-sm">
                        <div className="flex items-center justify-between mb-3">
                          <div className="flex items-center gap-2">
                            <Target className="w-5 h-5 text-amber-700" />
                            <span className="font-bold text-amber-900 text-sm tracking-wide">PREMIUM OFFER</span>
                          </div>
                          <Badge className="bg-gradient-to-r from-amber-600 to-yellow-600 text-white font-bold text-lg px-3 py-1 shadow-sm">
                            {discountCodes[firm.provider_name].percentage}% OFF
                          </Badge>
                        </div>
                        <div className="space-y-2">
                          <div className="text-sm font-medium text-amber-800">
                            {discountCodes[firm.provider_name].description}
                          </div>
                          <div className="flex items-center justify-between gap-2">
                            <code className="text-base bg-white text-amber-900 px-3 py-2 rounded border border-amber-200 font-mono font-bold shadow-sm flex-1 text-center">
                              {discountCodes[firm.provider_name].code}
                            </code>
                            <Button size="sm" className="bg-gradient-to-r from-amber-600 to-yellow-600 hover:from-amber-700 hover:to-yellow-700 text-white shadow-sm whitespace-nowrap">
                              Apply Code
                            </Button>
                          </div>
                        </div>
                    ...
                    {/* Action Button */}
                    <Button
                      className="w-full bg-gray-900 hover:bg-gray-800 text-white font-semibold shadow-sm transition-all duration-200"
                    ...
                    <div className="flex gap-1 pt-2 border-t border-gray-100">
                      <Button
                        variant="ghost"
                        size="sm"
                        className="flex-1 text-xs h-8 text-gray-600 hover:text-gray-900 hover:bg-gray-50"
                        onClick={() => {
