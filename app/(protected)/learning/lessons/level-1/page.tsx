                ...
                <div className="grid grid-cols-1 gap-3">
                  {lessonData.objectives.map((objective, index) => (
                    <div key={index} className="flex items-start gap-3 p-4 bg-success/5 border border-success/20 rounded-lg">
                      ...
                      <span className="text-success-foreground font-medium">{objective}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="text-center space-y-4">
                <div className={`text-8xl font-bold mb-4 ${
                  score === lessonData.assessment.questions.length
                    ? 'text-success'
                    : score >= lessonData.assessment.questions.length * 0.7
                    ? 'text-warning'
                    : 'text-destructive'
                }`}>
                  {Math.round((score / lessonData.assessment.questions.length) * 100)}%
                </div>
                <div className={`w-32 h-32 mx-auto rounded-full flex items-center justify-center shadow-xl ${
                  score === lessonData.assessment.questions.length
                    ? 'bg-success'
                    : score >= lessonData.assessment.questions.length * 0.7
                    ? 'bg-warning'
                ...
                {lessonData.assessment.questions.map((question, index) => (
                  <Card key={question.id} className={`shadow-md ${
                    selectedAnswers[index] === question.correctAnswer
                      ? 'border-secondary-foreground/30 bg-secondary/60'
                      : 'border-muted-foreground/30 bg-muted/60'
                  }`}>
                    <CardContent className="p-6">
                      <div className="flex items-start gap-4">
                        {selectedAnswers[index] === question.correctAnswer ? (
                          <CheckCircle className="w-7 h-7 text-secondary-foreground mt-1 flex-shrink-0" />
                        ) : (
                          <XCircle className="w-7 h-7 text-muted-foreground mt-1 flex-shrink-0" />
                        )}
                          ...
                          <div className="space-y-1">
                            <div className="text-xs text-slate-500">Your answer:
                              <span className={`ml-2 font-medium ${
                                selectedAnswers[index] === question.correctAnswer ? 'text-secondary-foreground' : 'text-muted-foreground'
                              }`}>
              ...
              <div className="pt-6 border-t border-slate-200">
                {score === lessonData.assessment.questions.length ? (
                  <div className="space-y-6">
                    ...
                    <Link href="/learning/gamified" className="block">
