...
export default function JawGnarlEntryDetailPage() {
  ...
  const getMoodColor = (mood?: any) => {
    ...
    switch (moodLabel) {
      ...
      case 'annoyed': return 'bg-orange-500/10 text-orange-600';
      case 'neutral': return 'bg-blue-500/10 text-blue-600';
      case 'optimistic': return 'bg-yellow-500/10 text-yellow-600';
  ...
  return (
    <div className="container mx-auto p-6 max-w-4xl">
      ...
      <div className="flex items-center gap-4 mb-6">
        <Button variant="outline" size="sm" onClick={() => router.push('/app/jawgnarl')}>
        ...
        <Card className="bg-gradient-to-r from-blue-50 to-purple-50 border-2">
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="text-center">
                <div className={`text-2xl font-bold ${entry.tradingStats.pnl >= 0 ? 'text-green-600' : 'text-red-600'}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium text-green-600">{entry.tradingStats.winningTrades}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Losing Trades:</span>
                <span className="font-medium text-red-600">{entry.tradingStats.losingTrades}</span>
              </div>
            </div>
          </CardContent>
        </Card>
            ...
            <CardContent className="space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <div className={`p-3 rounded-lg border ${entry.reflection.planFollowed ? 'border-green-200 bg-green-50' : 'border-red-200 bg-red-50'}`}>
                    <div className="flex items-center gap-2">
                      <div className={`w-3 h-3 rounded-full ${entry.reflection.planFollowed ? 'bg-green-500' : 'bg-red-500'}`}></div>
                      <span className="text-sm font-medium">Followed Plan</span>
                    </div>
                  </div>
                  <div className={`p-3 rounded-lg border ${entry.reflection.riskManaged ? 'border-green-200 bg-green-50' : 'border-red-200 bg-red-50'}`}>
                    <div className="flex items-center gap-2">
                      <div className={`w-3 h-3 rounded-full ${entry.reflection.riskManaged ? 'bg-green-500' : 'bg-red-500'}`}></div>
                      <span className="text-sm font-medium">Risk Managed</span>
                    </div>
                  </div>
                  <div className={`p-3 rounded-lg border ${entry.reflection.learnedSomething ? 'border-green-200 bg-green-50' : 'border-red-200 bg-red-50'}`}>
                    <div className="flex items-center gap-2">
                      <div className={`w-3 h-3 rounded-full ${entry.reflection.learnedSomething ? 'bg-green-500' : 'bg-red-500'}`}></div>
